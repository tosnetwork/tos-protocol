...
func equalUnixMillisNanos(millis, nanos int64) bool {
	converted, ok := unixMillisToNanos(millis, false)
	return ok && converted == nanos
}

func unixMillisToNanos(millis int64, allowZero bool) (int64, bool) {
	if millis < 0 || (!allowZero && millis == 0) || millis > math.MaxInt64/int64(time.Millisecond) {
		return 0, false
	}
	return millis * int64(time.Millisecond), true
}
