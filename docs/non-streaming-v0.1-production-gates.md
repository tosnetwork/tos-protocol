...
The [local GPU CDI closeout](local-gpu-cdi-execution-closeout-2026-08-02.md)
then binds that lease to the production containerd CDI injector and passes both
the complete lifecycle suite and exact MOCK-device injection through real
containerd/runc. The same code supplies a fail-closed physical NVIDIA
certification entry point; this CPU host cannot produce that hardware evidence.
...
| PG-02 | Live chain authority | Partial | Deploy reviewed Agent Account/service contracts behind at least three independent RPC endpoints and demonstrate controller rotation, client-key revocation, stale-node rejection, finality and payment-reorganization behavior. | The [local M1 report](local-three-node-ai-edge-m1-evidence-2026-08-01.md) proves three-node strict-majority authority, client-key resolution and finalized native payment. Automated fault tests cover controller/current-digest rotation, key revocation, stale/high-water rejection and payment reorganization; the corresponding live production ceremony across independent endpoints remains external. | 2026-08-02 |
...
| PG-06 | Tier 1 NVIDIA terminal | Open | Certify the supported driver/runtime matrix, model load, cold start, sustained inference, thermal/power behavior and owner-priority operation on the selected Linux/NVIDIA terminal class. | MOCK telemetry and end-to-end admission cover VRAM, thermal/power degradation, GPU loss and recovery. The production operator-alias lease now performs exact CDI injection, and `make nvidia-certification` supplies a fail-closed physical test. This host has no NVIDIA device, so the driver/runtime/performance/thermal evidence remains external. See the [GPU CDI closeout](local-gpu-cdi-execution-closeout-2026-08-02.md). | 2026-08-02 |
| PG-07 | Physical execution isolation | Partial | Run lifecycle and cleanup tests on the exact kernel, cgroup v2, containerd, runc, seccomp, namespace, filesystem and NVIDIA device configuration. Prove no residual workload objects after success, failure, cancellation or restart. | The [2026-08-02 closeout](local-host-closeout-2026-08-02.md) passed the complete CPU lifecycle suite on real local containerd/runc. The [GPU CDI closeout](local-gpu-cdi-execution-closeout-2026-08-02.md) additionally passed exact MOCK-device OCI injection, execution and cleanup through real containerd/runc. Target-kernel adversarial/reboot evidence and physical NVIDIA isolation remain uncertified. | 2026-08-02 |
| PG-08 | Model and update supply chain | Partial | Provision real trust roots and signed artifacts; rehearse corruption, incompatible runtime, interrupted activation, disk full, power loss, anti-rollback, known-good rollback and disconnected operation on target hardware. | Signed staging, exact candidate-boot health, rollback, anti-rollback, tamper handling and authenticated bounded lifecycle history pass automated race tests. Deterministic `ENOSPC` injection proves manifest/state writes fail closed without state advance or temporary residue and retry cleanly. Target trust-root ceremony and physical power-loss evidence remain external. | 2026-08-02 |
| PG-09 | Availability and bounded memory | Partial | Run sustained bounded-concurrency, slow-client, malformed-input, chain/Worker/signer outage, disk-quota and restart tests while recording RSS, heap, goroutines, file descriptors, task store, bbolt, RAM, VRAM and cache behavior to steady state. | Race tests, TLS test and an additional 30,000-request local sample passed with stable RSS, descriptors and database sizes. The [2026-08-02 MOCK closeout](local-mock-and-conformance-closeout-2026-08-02.md) additionally records typed-nil rejection, resolver/Worker/hardware panic containment, cancellation-late success rejection, deterministic disk-full persistence failure, repeated race tests and more than 12.9 million cumulative strict-parser fuzz inputs; this is not a long-duration target-host leak certificate. | 2026-08-02 |
